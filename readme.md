...
## gRPC performance issues.
 ...
 ## Using Protobuf to improve gRPC performance 
...
- Server **HelloWorldServiceImpl**
  PersonalInfo POJO that implements the PersonalInfo class that was generated from the **PersonalInfo.proto** file. We override the sayHello() method and generate a Greeting response based on the first and last name of the Person passed in the request.
  ...
  The PersonalInfo POJO is annotated with @GRpcService which auto-configures the specified gRPC service to be exposed on port **6565**

- Client **PersonalInfoClient**
  ...
  We use an init() method annotated with @PostConstruct in order to build a new MessageChannel right after the after the bean has been initialized. The channel is then used to create the PersonalInfoBlockingStub stub.
...
public class GrpcApplicationTests {
	...
	@Autowired
	  ...
	  public void testGetPersonalInfo() {
	    assertThat(PersonalInfo.sayPersonalInfo("Kurt", "Ellington"))
	        .isEqualTo("Hello Kurt Ellington!");
	  }
...
- Run test or hit api
http://localhost:8080/sayhello?firstname=Kurt&lastname=Elllington
...
[INFO] Finished at: 2021-10-18T14:32:50+05:30
